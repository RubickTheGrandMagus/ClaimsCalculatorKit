<script lang="ts">
    ...
    tppd.pagi = (tppd.pagi>5)? 5:tppd.pagi;
    let rankHigher:boolean = $state((tppd.retrank=="Select Your Rank" || tppd.rank==tppd.retrank)? false:true);
    ...
    function computeHSR(){
        ...
        let index = salaryGrade.findIndex(t=>t.rank == tppd.rank);
        let index2 = longevityPay.findIndex(t=>t.pagi == tppd.pagi);
        if(rankHigher && index<salaryGrade.length-1 && tppd.rank!="Your current rank"){
            index +=1;
        }
